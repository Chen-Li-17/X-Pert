...
class MLPDecoder(nn.Module):
    """
    三层 MLP 解码器：
        latent_dim → hidden_dim1 → hidden_dim2 → out_dim
    """

    def __init__(self, latent_dim: int, hidden_dim1: int,
                 hidden_dim2: int, out_dim: int, p_dropout: float = 0.0):
        super().__init__()
    ...
    def forward(self, z: torch.Tensor) -> torch.Tensor:
        ...
        -------
        z : torch.Tensor
            隐层向量 (batch_size, latent_dim)
        返回
        -------
        torch.Tensor
            预测结果 (batch_size, out_dim)
        """
        x = F.relu(self.fc1(z))
        x = self.dropout(x)
        ...
        out = self.fc3(x)         # 若需要 sigmoid/softmax，请在外部加
        return out
...
class FiLM(nn.Module):
    def __init__(self, d_model, d_pert, groups=1, mlp_ratio=2,
                 ff_mult = 4):
        super().__init__()
        hidden = d_pert * mlp_ratio
        self.mlp = nn.Sequential(
            nn.Linear(d_pert, hidden),
            nn.SiLU(),
            nn.Linear(hidden, 2 * d_model)  # 输出 [gamma||beta]
        )
        self.groups = groups
        if groups > 1:
            self.group_norm = nn.GroupNorm(groups, d_model, affine=False)
        else:
            self.group_norm = nn.Identity()
        self.ff = FeedForward(d_model, mult = ff_mult)
    ...
    def forward(self, x, p_global):
        """
        x: (B,N,D)
        p_global: (B,Dp)  ->  2D  -> gamma,beta
        """
        g_beta = self.mlp(p_global)  # (B, 2D)
        B, N, D = x.shape
        gamma, beta = g_beta.chunk(2, dim=-1)  # (B,D)
        # x_norm = self.group_norm(x)
        x_norm = x
        x = (1 + gamma[:, None, :]) * x_norm + beta[:, None, :]
...
class BCE_MSE_Decoder(nn.Module):
    r"""
    Multi-task decoder WITHOUT shared hidden layers.

    输入:
        x — (B, L, d_in)  transformer 输出或 token 表征

    输出字典:
        logits_exist — (B, L)  预测 y>0 的 logits   → BCE
        expr_pred    — (B, L)  预测 log1p(count)   → MSE (仅 y>0)
    """
    def __init__(
        self,
        d_model: int,
        d_hidden_exist: int = 256,
        d_hidden_expr:  int = 256,
        dropout: float = 0,
        use_batch_labels: bool = False,
    ):
        super().__init__()
        ...
        # ------- ① 0/1 存在性支路 -------
        self.exist_branch = nn.Sequential(
            nn.Linear(d_in, d_model),
            nn.LeakyReLU(),
            nn.Dropout(dropout),
            nn.Linear(d_model, d_model),
        ...
        self.exist_head = nn.Linear(d_model, 1)   # logits
        ...
        self.expr_head = nn.Linear(d_model, 1)     # 连续值

        # ---- 初始化小技巧 ----
        nn.init.constant_(self.exist_head.bias, -2.)  # 初始 P≈0.12
        nn.init.xavier_uniform_(self.expr_head.weight)

    # forward
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        # (B, L, d_in) → (B, L, *)
        exist_latent = self.exist_branch(x)
        expr_latent  = self.expr_branch(x)

        logits_exist = self.exist_head(exist_latent).squeeze(-1)  # (B, L)
        expr_pred    = self.expr_head(expr_latent).squeeze(-1)    # (B, L)
        return {"logits_exist": logits_exist,
                "expr_pred":    expr_pred}

class ZILNDecoder(nn.Module):
    r"""
    Zero-Inflated Log-Normal decoder for log1p(counts).

    返回:
        • mu          — 正态均值 (B, L)
        • log_sigma   — log(σ) ，保证 σ>0  (B, L)
        • pi_logits   — 结构零 logits (B, L)
    """
    def __init__(self, d_model: int, use_batch_labels: bool = False,
        ...
        self.backbone = (
            ...
            if shared_hidden else nn.Identity()
        )
        h_dim = d_model if shared_hidden else d_in

        self.mu_head        = nn.Linear(h_dim, 1)   # μ ∈ ℝ
        self.log_sigma_head = nn.Linear(h_dim, 1)   # σ > 0  →  exp(logσ)
        self.pi_head        = nn.Linear(h_dim, 1)   # logits

    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        h = self.backbone(x)                                     # (B, L, h_dim)
        mu        = self.mu_head(h).squeeze(-1)                  # (B, L)
        log_sigma = self.log_sigma_head(h).squeeze(-1)           # (B, L)
        pi_logits = self.pi_head(h).squeeze(-1)                  # (B, L)
        return dict(mu=mu, log_sigma=log_sigma, pi_logits=pi_logits)
    
class DoseFiLM(nn.Module):
    """
    将 dose → (γ, β)，用于 FiLM 调制。
    
    参数
    ----
    d_model     : 药物 / 基因表达 embedding 维度
    hidden_dim  : MLP 隐层维度，越大越能拟合复杂剂量-反应曲线
    act         : 隐层激活函数，常用 SiLU/GELU/ReLU
    """
    def __init__(self,
                 d_model: int,
                 hidden_dim: int = 32,
        ...
        Act = {"relu": nn.ReLU, "gelu": nn.GELU,
               "silu": nn.SiLU}[act.lower()]

        # 两层 MLP：dose → hidden → 2*d_model
        self.mlp = nn.Sequential(
            nn.Linear(1, hidden_dim),  # 若剂量已向量化，可把 1 改成 dose_dim
            Act(),
            nn.Linear(hidden_dim, 2 * d_model)
        )

        # 可选：初始化 γ 为 1、β 为 0，利于早期稳定训练
        with torch.no_grad():
            self.mlp[-1].weight.zero_()
            self.mlp[-1].bias[:d_model].fill_(1.0)  # γ 起始≈1
            self.mlp[-1].bias[d_model:].zero_()     # β 起始≈0

    def forward(self, dose: torch.Tensor):
        """
        dose: (B, 1) 或 (B, dose_dim) —— 建议使用 log10(dose+ε) 再送入
        返回:
            γ, β: (B, 1, d_model) 方便广播到 (B, L, d_model)
        """
        out = self.mlp(dose)                       # (B, 2*d_model)
        gamma, beta = out.chunk(2, dim=-1)         # 各 (B, d_model)
        gamma = F.softplus(gamma) + 1e-4           # 保证 γ>0，避免梯度消失
        # reshape 便于与 token 级别 embedding (B, L, d_model) 广播
